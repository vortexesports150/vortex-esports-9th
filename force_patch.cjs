...
const target1 = `                           <div className="flex items-center gap-2 flex-1 justify-end max-w-[180px]">
                           ...
                           <span className="text-fuchsia-500 text-[10px] font-black uppercase tracking-widest px-2 py-0.5 bg-fuchsia-950/80 rounded border border-fuchsia-500/30">VS</span>
                           <div className="flex items-center gap-2 flex-1 justify-start max-w-[180px]">
                             {squad2 ? (
...
const repl1 = `                           <div className="flex flex-col items-center gap-1.5 flex-1 justify-center max-w-[180px]">
                             {squad1 ? (
                           ...
                           <span className="text-fuchsia-500 text-[10px] font-black uppercase tracking-widest px-3 py-1 bg-fuchsia-950/80 rounded-md border border-fuchsia-500/30 shrink-0 mx-2">VS</span>
                           <div className="flex flex-col items-center gap-1.5 flex-1 justify-center max-w-[180px]">
                             {squad2 ? (
...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('                           <div className="flex items-center gap-2 flex-1 justify-end max-w-[180px]">')) {
    // Found the start of squad1
    console.log("Replacing at line", i);
    // Erase the old block
    lines.splice(i, 32, repl1);
    break;
  }
}

fs.writeFileSync('src/UpazilaLeagueUser.tsx', lines.join('\n'));
console.log("Done group stage force replace");
